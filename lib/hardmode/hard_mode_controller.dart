...
class HardModeController extends GetxController {
  ...
  Future<void> _captureAndAnalyze() async {
    try {
      ...
      if (faces.isEmpty) {
        _message.value = 'No face detected. Please try again.';
      ...
      if (isExpressionCorrect) {
        ...
        if (userId != null) {
          await saveStatistics(userId, _currentExpression.value, 1, 0);
        }
        Get.off(() => StageClear1());
      } else {
        _message.value = 'Cheer Up...!';
        if (userId != null) {
          await saveStatistics(userId, _currentExpression.value, 0, 1);
        }
        Get.off(() => StageFail());
      }
    } catch (e) {
  ...
  bool _isEyeClosed(Face face) {
    ...
    return leftEyeOpen != null && rightEyeOpen != null && leftEyeOpen < 0.2 &&
        rightEyeOpen < 0.2;
  }
  ...
  bool _isMouthOpen(Face face) {
    ...
    if (leftMouth != null && rightMouth != null && bottomMouth != null &&
      ...
      double mouthWidth = (rightMouth.position.x.toDouble() -
          leftMouth.position.x.toDouble()).abs();
      double mouthHeight = (bottomMouth.position.y.toDouble() -
          upperMouth.position.y.toDouble()).abs();
  ...
  bool _isFrowning(Face face) {
    ...
    if (leftEyebrowTop != null && rightEyebrowTop != null && leftEye != null &&
        rightEye != null) {
      double leftEyebrowY = leftEyebrowTop.points.map((p) => p.y.toDouble())
          .reduce((a, b) => a + b) / leftEyebrowTop.points.length;
      double rightEyebrowY = rightEyebrowTop.points.map((p) => p.y.toDouble())
