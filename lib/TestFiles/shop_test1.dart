...
class ShopState extends State<Shop2> {
  ...
  Future<void> _fetchUserCoins() async {
    ...
    userId = prefs.getString('userId') ?? ''; // 로그인 ID 가져오기
  ...
  Future<void> _purchaseItems() async {
    ...
    final url = Uri.parse('http://34.47.88.29:8082/api/purchase/purchase');
    try {
      final response = await http.post(
        url,
        headers: {'Authorization': 'Bearer $accessToken', 'Content-Type': 'application/json'},
        body: jsonEncode({'Id': userId, 'ItemId': purchasedItems}),
      );
